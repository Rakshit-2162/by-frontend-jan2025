...
class AccountServices:
    ...
    async def get_all_accounts(self):
        accounts = await self.collection.find().to_list()
    ...
    async def deposit(self, id: int, amount: float, description: str):
        ...
        await self.collection.update_one({"id": id}, {"$set": {"balance": new_balance, "updateTimeStamp": datetime.now()}})
    ...
    async def withdraw(self, id: int, amount: float, description: str):
        ...
        transaction = Transaction(
            id=int(datetime.now().timestamp()),  # Unique transaction ID based on timestamp
